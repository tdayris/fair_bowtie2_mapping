module bowtie2_sambamba:
    meta_wrapper: f"{snakemake_wrappers_version}/meta/bio/bowtie2_sambamba"
...
use rule bowtie2_alignment from bowtie2_sambamba with:
    ...
    params:
        extra=config.get("params", {}).get("bowtie2", {}).get("align", " --rg-id {sample} --rg 'SM:{sample} LB:{sample} PU:{species}.{build}.{release}.{datatype}.{sample} PL:ILLUMINA'")
...
use rule sambamba_view from bowtie2_sambamba with:
    ...
    params:
        extra=config.get("params", {}).get("sambamba", {}).get("view", "--format 'bam' --filter 'mapping_quality >= 30 and not (unmapped or mate_is_unmapped)' ")
...
use rule sambamba_markdup from bowtie2_sambamba with:
    ...
    params:
        extra=config.get("params", {}).get("sambamba", {}).get("markdup", " --remove-duplicates ")
